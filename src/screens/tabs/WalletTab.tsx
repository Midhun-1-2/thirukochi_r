...
export default function WalletTab() {
  const earned = useCountUp(referral.bonusEarned, formatRupees, { duration: 1.6, delay: 0.25 });
  const grams = useCountUp(wallet.goldGrams, formatGrams, { duration: 1.6, delay: 0.35 });
    ...
    >
      ...
      <div className="lg:col-span-2">
        <GlassCard className="px-6 py-7" beamDelay={0}>
          ...
          <p
            ref={earned as React.RefObject<HTMLParagraphElement>}
            className="mt-3 font-display text-[40px] leading-none text-metal-gold"
          />
          ...
          <p className="mt-5 text-[11px] text-champagne-dim">
            Holding <span ref={grams as React.RefObject<HTMLSpanElement>} /> · {wallet.scheme.name} instalment{" "}
            {wallet.scheme.paid} of {wallet.scheme.total}
          </p>
