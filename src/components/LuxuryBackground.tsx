...
function LuxuryBackgroundBase() {
  ...
  return (
    // One composited layer for the whole atmosphere, so the glass surfaces
    // above sample a GPU texture instead of re-rasterising this subtree.
    <div
      aria-hidden
      className="pointer-events-none absolute inset-0 overflow-hidden bg-wine-950"
      style={{ transform: "translateZ(0)", isolation: "isolate", contain: "strict" }}
    >
      {/* Photographic gold-ribbon plate, drifting very slowly */}
      <div
        className={`absolute inset-0 will-change-transform ${reduced ? "" : "bg-drift"}`}
        style={{
      ...
      {orbs.map((orb, i) => (
        <div
          key={i}
          className={`absolute rounded-full will-change-transform ${reduced ? "" : "orb-drift"}`}
          style={{
            width: orb.size,
            height: orb.size,
            left: orb.x,
            ...
            filter: "blur(48px)",
            "--drift-x": `${orb.drift[0]}px`,
            "--drift-y": `${orb.drift[1]}px`,
      ...
      {!reduced &&
        particles.map((p, i) => (
          <span
            key={i}
            className="particle-float absolute rounded-full will-change-transform"
            style={{
              left: p.left,
              top: p.top,
              width: p.size,
      ...
      {/*
        Plain compositing rather than `mix-blend-overlay`: a blend mode here
        costs a viewport-wide pass on every frame, which is what made large
        windows chug while the phone frame stayed smooth. At this opacity the
        two read the same over the dark ground.
      */}
      <div
